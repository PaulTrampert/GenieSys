...
GenieSys::BCLR::BCLR(GenieSys::M68kCpu *cpu, GenieSys::Bus *bus) : GenieSys::CpuOperation(cpu, bus) {

}

std::vector<uint16_t> GenieSys::BCLR::getOpcodes() {
...
uint8_t GenieSys::BCLR::getSpecificity() {
    return DnMask.getWidth() + ImmMask.getWidth() + EaModeMask.getWidth() + EaAddrMask.getWidth();
}

uint8_t GenieSys::BCLR::execute(uint16_t opWord) {
    uint8_t cycles;
    ...
    if (imm) {
        ...
        cycles = eaModeId == GenieSys::DataRegisterDirectMode::MODE_ID ? 14 : 12;
    }
    else {
...
std::string GenieSys::BCLR::disassemble(uint16_t opWord) {
    ...
    if (immMode) {
        ...
        stream << "BCLR " << imm->disassemble(GenieSys::ImmediateDataMode::MODE_ID, 1) << "," << destData;
    }
    else {
